...
{$ENDIF}
    Pointer(_EnumProcesses) := GetProcAddress(hPSAPI, 'EnumProcesses');
    Pointer(_EnumProcessModules) := GetProcAddress(hPSAPI, 'EnumProcessModules');
    {procedure}Pointer(_GetModuleBaseName) := GetProcAddress(hPSAPI, 'GetModuleBaseNameW');
    {procedure}Pointer(_GetModuleFileNameEx) := GetProcAddress(hPSAPI, 'GetModuleFileNameExW');
    {procedure}Pointer(_GetModuleBaseNameA) := GetProcAddress(hPSAPI, 'GetModuleBaseNameA');
    {procedure}Pointer(_GetModuleFileNameExA) := GetProcAddress(hPSAPI, 'GetModuleFileNameExA');
    {procedure}Pointer(_GetModuleBaseNameW) := GetProcAddress(hPSAPI, 'GetModuleBaseNameW');
    {procedure}Pointer(_GetModuleFileNameExW) := GetProcAddress(hPSAPI, 'GetModuleFileNameExW');
    Pointer(_GetModuleInformation) := GetProcAddress(hPSAPI, 'GetModuleInformation');
    Pointer(_EmptyWorkingSet) := GetProcAddress(hPSAPI, 'EmptyWorkingSet');
    Pointer(_QueryWorkingSet) := GetProcAddress(hPSAPI, 'QueryWorkingSet');
    Pointer(_InitializeProcessForWsWatch) := GetProcAddress(hPSAPI, 'InitializeProcessForWsWatch');
    {procedure}Pointer(_GetMappedFileName) := GetProcAddress(hPSAPI, 'GetMappedFileNameW');
    {procedure}Pointer(_GetDeviceDriverBaseName) := GetProcAddress(hPSAPI, 'GetDeviceDriverBaseNameW');
    {procedure}Pointer(_GetDeviceDriverFileName) := GetProcAddress(hPSAPI, 'GetDeviceDriverFileNameW');
    {procedure}Pointer(_GetMappedFileNameA) := GetProcAddress(hPSAPI, 'GetMappedFileNameA');
    {procedure}Pointer(_GetDeviceDriverBaseNameA) := GetProcAddress(hPSAPI, 'GetDeviceDriverBaseNameA');
    {procedure}Pointer(_GetDeviceDriverFileNameA) := GetProcAddress(hPSAPI, 'GetDeviceDriverFileNameA');
    {procedure}Pointer(_GetMappedFileNameW) := GetProcAddress(hPSAPI, 'GetMappedFileNameW');
    {procedure}Pointer(_GetDeviceDriverBaseNameW) := GetProcAddress(hPSAPI, 'GetDeviceDriverBaseNameW');
    {procedure}Pointer(_GetDeviceDriverFileNameW) := GetProcAddress(hPSAPI, 'GetDeviceDriverFileNameW');
    Pointer(_EnumDeviceDrivers) := GetProcAddress(hPSAPI, 'EnumDeviceDrivers');
    Pointer(_GetProcessMemoryInfo) := GetProcAddress(hPSAPI, 'GetProcessMemoryInfo');
  end;
  Result := True;
end;
